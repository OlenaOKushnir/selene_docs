...
Just as for SeleneElement for SeleneCollection there are condition checks(```should(be.Condition)``` or ```should(have.Condition)```). As a result, the SeleneCollection object is returned, and this also allows you to build chains of calls.

The SeleneCollection class also has a special type of check - the correspondence of each individual element from the list to a specific condition is checked(```should_each(be.Condition)``` or ```should_each(have.Condition)```):
...
These methods also have built-in explicit waits. Within 4 seconds (by default) they wait for the condition to be satisfied.

## Methods to get statues and attributes of elements collection

You can use such methods to get statues and attributes of SeleneCollection:
...
```element_by(Condition)``` | ```find_by(Condition)```

returns the first collection element that satisfied the condition
